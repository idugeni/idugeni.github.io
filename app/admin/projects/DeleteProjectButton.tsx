...
export function DeleteProjectButton({ projectId, projectName }: DeleteProjectButtonProps) {
  ...
  const handleDelete = async () => { setIsPending(true); try { await deleteProject(projectId); toast({ title: "Project deleted successfully" }); router.refresh(); setOpen(false); } catch (error) { toast({ title: "Error", description: error instanceof Error ? error.message : "Failed to delete project", variant: "destructive" }); } finally { setIsPending(false); } };
