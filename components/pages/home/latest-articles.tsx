    ...
    <section className="py-24 bg-card relative section-fade">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {!articles || articles.length === 0 ? (
          <SectionEmptyState
            title="NO_ARTICLES_PUBLISHED_YET"
            description="Blog articles will appear here once published."
            icon="◈"
          />
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {articles.map((article, i) => (
              <div key={article.id} className="sr-reveal" style={{ transitionDelay: `${i * 150}ms` }}>
                <Link href={`/blog/${article.slug}`} prefetch={false}>
                  <div className="glass-card h-full flex flex-col overflow-hidden group cursor-pointer hover:shadow-[0_0_20px_rgba(6,182,212,0.1)] transition-shadow">
                    ...
                    <div className="p-5 flex-1 flex flex-col">
                      ...
                      <h3 className="font-orbitron font-bold text-sm mb-2 group-hover:text-primary transition-colors line-clamp-2">{article.judul}</h3>
                      <p className="text-muted-foreground font-mono text-xs line-clamp-2 flex-1">{article.ringkasan}</p>
