...
export function NewsletterFilters({ filters }: { filters: NewsletterFiltersState }) {
  return <form action="/admin/newsletter" className="grid gap-3 rounded-none border border-border/50 bg-card/70 p-4 md:grid-cols-5">
    ...
    <div className="flex gap-2"><Button className="flex-1 rounded-none font-mono" type="submit">FILTER</Button><Button asChild variant="outline" className="rounded-none font-mono"><Link href="/admin/newsletter" prefetch={false}>RESET</Link></Button></div>
