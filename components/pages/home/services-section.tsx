      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="sr-reveal">
          <div className="text-center mb-16">
            ...
            <p className="text-muted-foreground font-mono text-sm max-w-2xl mx-auto">Solusi end-to-end untuk kebutuhan digital Anda. Dari konsep hingga deployment.</p>
          </div>
        </div>
        ...
        {!services || services.length === 0 ? (
          <SectionEmptyState
            ...
            description="Services will appear here once configured in the admin panel."
            icon="△"
          />
        ) : (
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {services.slice(0, 6).map((service, i) => (
                <div key={service.id} className="sr-reveal" style={{ transitionDelay: `${i * 100}ms` }}>
                  <div className="relative group h-full">
                    ...
                    <div className="relative bg-background border border-border/50 group-hover:border-primary/40 p-6 h-full flex flex-col transition-all duration-300">
                      ...
                      <div className="font-orbitron text-[10px] text-primary/40 mb-4">0{i + 1}</div>
                      <h3 className="font-orbitron font-bold text-sm mb-3 group-hover:text-primary transition-colors">{service.nama}</h3>
                      <p className="text-muted-foreground font-mono text-xs leading-relaxed mb-5 flex-1">{service.deskripsiPendek}</p>
                      {service.hargaMulai && (
                        <div className="pt-4 border-t border-border/30 flex items-center justify-between">
                          <span className="font-mono text-[10px] text-muted-foreground/60">STARTING_AT</span>
                          <span className="font-mono text-sm text-primary font-bold">{service.hargaMulai}</span>
                        </div>
                      )}
            ...
            <div className="sr-reveal" style={{ transitionDelay: "400ms" }}>
