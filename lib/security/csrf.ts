...
const CSRF_SECRET = process.env.SUPABASE_SECRET_KEY

if (process.env.NODE_ENV === "production" && !CSRF_SECRET) {
  throw new Error("SUPABASE_SECRET_KEY must be set in production for CSRF protection")
}

function getCSRFSecret(): string {
  if (!CSRF_SECRET) throw new Error("SUPABASE_SECRET_KEY is not configured for CSRF protection")
  return CSRF_SECRET
}
